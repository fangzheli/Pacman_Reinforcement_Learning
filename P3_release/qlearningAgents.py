...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        actions = self.getLegalActions(state)
        if actions:
            return actions[[self.getQValue(state, action) for action in actions].index(max([self.getQValue(state, action) for action in actions]))]
        else:
            return None
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in features:
            pseudoQ += self.getWeights()[feature] * features[feature]
        return pseudoQ
